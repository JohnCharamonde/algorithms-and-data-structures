...
var maxSubArray = function(nums) {
  ...
  // FUNC: use start, end indices to recurse through nums, calculate tempSums, and check against max until all possible subarrays have been accounted for
  let recurse = (i, j) => {
    // calculate subarray sum as tempSum and check against max
    let tempSum = nums.slice(i, j + 1).reduce((acc, ele) => { return acc += ele});
    if(tempSum > max) max = tempSum;

    // Possible to increment end without exceeding nums length? Then do so.
...
// O: number = max sum for subarray
// I: array of nums
// C: O(n)
// E: empty array => null, account for negative sums

// 1. [-2]                               i = 0, j = 0, tempSum = -2, max = -2
// 2. [-2, 1]                            i = 0, j = 1, tempSum = -1, max = -1
// 3. [-2, 1, -3]                        i = 0, j = 2, tempSum = -4, max = -1
// 4. [-2, 1, -3, 4]                     i = 0, j = 3, tempSum = 0,  max = 0
// 5. [-2, 1, -3, 4, -1]                 i = 0, j = 4, tempSum = -1, max = 0
// 6. [-2, 1, -3, 4, -1, 2]              i = 0, j = 5, tempSum = 1,  max = 1
// 7. [-2, 1, -3, 4, -1, 2, 1]           i = 0, j = 6, tempSum = 2,  max = 2
// 8. [-2, 1, -3, 4, -1, 2, 1, -5]       i = 0, j = 7, tempSum = -3, max = 2
// 9. [-2, 1, -3, 4, -1, 2, 1, -5, 4]    i = 0, j = 8, tempSum = 1,  max = 2

// 10. [1]                               i = 1, j = 1, tempSum = 1,  max = 2
// 11. [1, -3]                           i = 1, j = 2, tempSum = -2, max = 2
// 12. [1, -3, 4]                        i = 1, j = 3, tempSum = 2,  max = 2
// 13. [1, -3, 4, -1]                    i = 1, j = 4, tempSum = 1,  max = 2
// 14. [1, -3, 4, -1, 2]                 i = 1, j = 5, tempSum = 3,  max = 3
// 15. [1, -3, 4, -1, 2, 1]              i = 1, j = 6, tempSum = 4,  max = 4
// 16. [1, -3, 4, -1, 2, 1, -5]          i = 1, j = 7, tempSum = -1, max = 4
// 17. [1, -3, 4, -1, 2, 1, -5, 4]       i = 1, j = 8, tempSum = 3,  max = 4

// 18. [-3]                              i = 2, j = 2, tempSum = -3, max = 4
// 19. [-3, 4]                           i = 2, j = 3, tempSum = 1,  max = 4
// 20. [-3, 4, -1]                       i = 2, j = 4, tempSum = 0,  max = 4
// 21. [-3, 4, -1, 2]                    i = 2, j = 5, tempSum = 2,  max = 4
// 22. [-3, 4, -1, 2, 1]                 i = 2, j = 6, tempSum = 3,  max = 4
// 23. [-3, 4, -1, 2, 1, -5]             i = 2, j = 7, tempSum = -2, max = 4
// 24. [-3, 4, -1, 2, 1, -5, 4]          i = 2, j = 8, tempSum = 2,  max = 4

// 25. [4]                               i = 3, j = 3, tempSum = 4,  max = 4
// 26. [4, -1]                           i = 3, j = 4, tempSum = 3,  max = 4
// 27. [4, -1, 2]                        i = 3, j = 5, tempSum = 5,  max = 5
// 28. [4, -1, 2, 1]                     i = 3, j = 6, tempSum = 6,  max = 6
// 29. [4, -1, 2, 1, -5]                 i = 3, j = 7, tempSum = 1,  max = 6
// 30. [4, -1, 2, 1, -5, 4]              i = 3, j = 8, tempSum = 5,  max = 6

// 31. [-1]                              i = 4, j = 4, tempSum = -1, max = 6
// 32. [-1, 2]                           i = 4, j = 5, tempSum = 1,  max = 6
// 33. [-1, 2, 1]                        i = 4, j = 6, tempSum = 2,  max = 6
// 34. [-1, 2, 1, -5]                    i = 4, j = 7, tempSum = -3, max = 6
// 35. [-1, 2, 1, -5, 4]                 i = 4, j = 8, tempSum = 1,  max = 6

// 36. [2]                               i = 5, j = 5, tempSum = 2,  max = 6
// 37. [2, 1]                            i = 5, j = 6, tempSum = 3,  max = 6
// 38. [2, 1, -5]                        i = 5, j = 7, tempSum = -2, max = 6
// 39. [2, 1, -5, 4]                     i = 5, j = 8, tempSum = 2,  max = 6

// 40. [1]                               i = 6, j = 6, tempSum = 1,  max = 6
// 41. [1, -5]                           i = 6, j = 7, tempSum = -4, max = 6
// 42. [1, -5, 4]                        i = 6, j = 8, tempSum = 0,  max = 6

// 43. [-5]                              i = 7, j = 7, tempSum = -5, max = 6
// 44. [-5, 4]                           i = 7, j = 8, tempSum = -1, max = 6

// 45. [4]                               i = 8, j = 8, tempSum = 1,  max = 6